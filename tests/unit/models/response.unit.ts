import * as ServerlessMochaPlugin from 'serverless-mocha-plugin';
import ResponseModel, { RESPONSE_HEADERS } from "../../../src/models/response.model";
...
describe('Model/ResponseModel', () => {
    ...
    describe('ensure messages are set correctly', () => {
        ...
        it('should return empty message field when a message is not supplied to it', () => {
            const response = new ResponseModel({}, 500);
            expect(JSON.parse(response.generate().body).message).to.eql("");
        });
